...
**当前状态**：两个源都真跑通了 —— GitHub Release 和 Cloudflare R2，
四个平台的包，打包 → 发包 → 翻指针 → 客户端查到 → 下载 → 校验 → 换入 →
重启生效，整条链路实测过。华为 OBS 是个**空着的槽**，不是待办，见第二节。
...
包放 **Cloudflare R2**，加上 GitHub Release —— 两个独立的源，
照抄官方"双源校验后才翻指针"那条。

`SOURCES` 里还留着一个 **OBS 的空槽**，没配。它不是待办事项，是个选项：
Cloudflare 在国内偶尔抖，而平台本身在华为云上，同厂链路更稳 —— 真遇到问题
了填五个值（前缀 `OBS_`，endpoint 得直接给，华为云没有"account id 拼域名"
那套）就变成三个源，代码一行不用改。在那之前它不占任何成本。
...
S3 兼容的源（R2、以及将来可能的 OBS）用同一段上传代码 —— 但**凭据和公开
域名必须各是各的**。变量名跟 Toonflow-app 对齐，同一个 Cloudflare 账号下两个项目长得一样，
值可以直接复用：
...
**现在只配了 R2。** `configured()` 按环境变量自动发现，配齐几个发几个；
一个都没配是硬失败 —— 静默地什么都不传、日志还写着"已发布"，是最糟的一种。
...
`publish()` 的第一件事是给每个源传一个 `.preflight` 小对象，再从
`*_PUBLIC_BASE` 读回来比一下内容 —— **确认这个域名服务的确实是我们要传的
那个桶**，然后才开始上传。

起因是一次真配错：`R2_PUBLIC_BASE` 指向的自定义域绑在**另一个项目的桶**上。
上传全部成功（走 S3 API，跟域名无关），指针也照翻，直到最后"公网回读"那步
才 404 —— 而那时线上的 manifest 已经指向一批公网取不到的包了。探针放在最
前面，这种配错在动任何东西之前就会被拦下，报错直接指出"这个域名多半绑在
另一个桶上"。
...
| 错法 | 后果 |
|---|---|
| 多个源共用一组 `AWS_ACCESS_KEY_ID` | 第二个源会拿第一个的 key 去认证，403 |
...
> **R2 已实测通过**（2026-09-09，v3.0.12.2 起）：探针 → 上传 → 回读校验 →
> 翻指针 → 公网回读，四个平台全过，本机从公开域名下包校验 sha256 一致。
...
公开仓的 Release 资产本来就是公网可下的不可变对象 —— 版本 tag 一发出，
资产就不再变，正好满足"包不可变"这一条。所以**桶还没开的时候升级链路就是
通的**，而且现在它仍然是两个源里的一个（客户端按顺序试，GitHub 排在前面，
R2 是国内那条）：
...
GET  /api/update/status     进度（idle/downloading/verifying/staged/applied/failed）
POST /api/update/apply      把解好的换进程序目录
```

**下载不占住调用方**：`download` 起个后台任务就返回，进度靠轮询 `status`。
和官方那套 ComfyUI 后端一样（入口秒回、worker 干活），理由也一样 ——
装的过程可能好几分钟，占住 HTTP 连接会让界面看起来卡死。

`download` 的参数（url / sha256 / version）**由调用方从 `check` 的结果原样带
过来**，后端不重新查清单：两次查之间清单可能翻了版本，于是用户点的是 A、
装上的是 B。

#### 换文件：改名挪开，不是直接覆盖

**Windows 不允许删除或覆盖正在运行的 exe，但允许给它改名。**

```text
ovgw.exe         → .old/ovgw.exe     旧的挪走（改名，允许）
.update/ovgw.exe → ovgw.exe          新的搬进来
```

`.old/` 留到**下次启动时才删** —— 那时它已经不在运行了。Unix 上直接覆盖也
行（inode 还活着），但用同一套路径能共用一份代码，失败时还能原样搬回来。

四项（`ovgw` / `ovagent` / `web` / `mcp`）**要么全换要么全不换**，中途失败
就回滚。换了一半的安装（新 ovgw 配旧 web）比完全没换更难查。

#### 几个刻意的决定

| | 为什么 |
|---|---|
| **换之前先验结构** | 包完好但布局变了（上游改了目录），照换会得到一个开不起来的安装，而用户原来那个是好的。缺项、类型不对、二进制小于 1 KB 一律中止 |
| **校验和不过就删掉整个 staging** | 留着的话下次点升级可能直接用了它 |
| **解包逐条查路径** | 包里的 `../` 会写到 staging 外面 —— 那就是程序目录。tar crate 自己有防护，但这是"写错了会被人利用"的地方，自己再挡一道 |
| **只认 https** | 允许 http 的话，能改 DNS 的人就能换包，而 sha256 是从同一条链路上拿的，挡不住 |
| **解包放阻塞线程池** | tar + gzip 是纯 CPU/IO，在异步 worker 上跑会卡住整个运行时几秒，表现是升级期间界面所有请求都没响应 |
| **`applied` 之后拒绝再下载** | 那时磁盘是新版、进程是旧版，唯一正确的下一步是重启。放行会冲掉"需要重启"的提示，而再 apply 一次会拿刚换上去的覆盖 `.old`，把原来能用的版本丢掉 |
| **staging 放程序目录旁边** | 换文件靠 rename，跨文件系统会失败（Linux 上 `/tmp` 常是 tmpfs） |

#### 不自动重启

想过，放弃了：旧进程还占着端口，新进程起来会绑定失败；要正确交接得再引一个
中间进程等端口释放。对一个本地工具不成比例 —— 如实返回 `restartRequired`，
让人自己重启。

（这条不是推测：实测时 `pkill -f` 没杀掉换过名的进程，新进程起来就撞上
`Address already in use`。）

**已实测**：装一份 3.0.12.2 的真包 → 从 R2 下载 → 校验 → 解包 → 换入 →
`.old/` 里是旧的 → 重启后 `--version` 变成新版 → 再启动时 `.old/` 被清掉。
校验和故意写错时，staging 被整个删掉、什么都没动。

## 六、还没定的

- 谁是主、谁是镜像（现在 github / r2 并列，客户端按名字排序依次试）
