...
description: |
  将产品图片制作成保持产品身份的极简广告短片，包含简洁英文文案、独立锚定图、节拍分镜和可选配乐替换。不用于口播广告、普通剪辑或复杂屏幕演示。
trigger-words: [极简产品广告, 高质感产品广告, 产品广告片, 电商产品视频, 新品发布广告, 苹果风产品广告]
---
...
1. **商品图和相关素材**
   ...
   - 如果用户已经提供素材，直接说明已检测到，不重复要求上传，只确认是否使用。
   - 如果有多张候选素材且角色不清，才请用户确认哪张作为主商品图。

2. **商品款式情况**
   - 确认是单款商品，还是多款式 / 多颜色 / 多型号商品。
   - 如果是多款式，确认哪个作为主推款。
   - 如果用户不指定，可根据产品图推荐一个主推款，但后续分镜表必须写成具体色名或具体款式名。
...
7. **视频模型策略**
   ...
   - 用户指定的模型生成失败时，最多针对性重试一次；然后说明能力差异，让用户选择 MiniMax H3 或其他等效模型，不要静默切换。

启动问询是强制 Gate。即使用户已经上传了商品素材，也只能跳过“请上传素材”这一步；不能跳过其余启动问询。进入产品分析、锚定图、分镜或视频生成前，仍须一次性确认商品款式、目标时长、画幅比例、苹果风模板和画面文案方式。视频模型不是启动问询问题：优先使用 MiniMax H3，并在已完成的启动结果中展示。如果用户说“你看着办 / 直接做”，可采用推荐默认值，但仍须先展示已采用的选择。
...
根据启动问询结果整理一份可执行制作简报，不要重复追问已经确认过的选择。简报用于后续叙事脊柱、文案、锚定图和分镜表。
...
- 已选苹果风模板。
- 单款 / 多款式策略。
- 画面文案方式：用户已有文案，或由流程生成英文 Apple 风文案。
...
- 若用户没有指定主推款，可选择视觉最稳的款式做主推款，其余款式做节奏和层次。
...
用户确认文案后，再进入锚定图生成。若用户授权“你看着办”，采用推荐文案继续。
...
根据已确认的三张独立锚定照片 + 精确节拍文字分镜表直接生成视频。视频风格、背景、光线、气质、节奏都跟随用户已选风格。视频比例和尺寸必须严格跟随用户选择，例如用户选择 16:9 就必须生成 16:9，不得自动改成其他比例。
...
- 镜间连贯硬约束。
- 说明是否需要原生音频，并写入已确认的音乐方向；默认使用原生音频，除非用户明确要求无声或其他模型。
- 禁止传原商品图。

## 步骤 9：独立科技配乐

优先使用 MiniMax H3 原生音频，因为它能在一次生成中对齐产品动作和声音。如果音乐不好听、太吵、太弱、不同步，或用户要求重新配乐，就用当前可用的音乐模型生成较长的独立器乐并替换音轨。选择付费或严格控时方案前，先确认明显的成本和输出差异。
...
- 音乐太差：生成较长的器乐替代版本，再剪取最佳连续片段，不强行锁死时长。
