...
description: 将旁白、故事节点、观点或抽象概念转化为经确认的纸拼贴静帧和带触感音效的定格片段，并可按需合成。不用于真人口播广告、精确可编辑图层、复杂排版或仅提示词任务。
trigger-words: [纸拼贴讲解, 纸拼贴动画, 半调拼贴, 拼贴科普, 定格拼贴, 拼贴动画]
---

# 纸拼贴讲解动画生成器

将一句配音/旁白、一个故事主题、观点句或抽象概念，转化为统一的编辑感纸拼贴动画序列。视觉语言是高级半调纸拼贴：大色块纸面、黑白半调照片剪影、选择性彩色卡纸点缀、暖白描边、柔和纸影、触感定格组装，以及清晰的拼贴音效。

该流程优先保证风格连续、色彩统一、纸张质感可控、定格拼贴节奏清楚，并采用明确的音频策略：默认保留或生成触感拼贴音效，但不添加 BGM、旁白/配音和字幕，除非用户要求。
...
- 默认不添加旁白/配音：可以询问用户是否需要旁白讲解/配音/旁白，但只有用户明确要求时才写稿、配音或加入人声
...
1. 在第一次制作方案确认时，必须明确写出默认媒体方式：保留或生成触感纸拼贴音效；默认不添加 BGM、旁白/配音和字幕，除非用户明确要求。
2. 对科普或讲解类内容，可以询问用户是否需要 **旁白讲解/配音/旁白**、**BGM** 或 **字幕**，但必须把它们作为可选增强，而不是默认项。
...
优先使用 MiniMax H3，因为它的原生音频能力适合本流程的同步拼贴音效。用户指定其他模型且满足已确认要求时遵循其选择；如果模型失败、不可用或缺少必要能力，说明差异并让用户选择等效方案。
...
- 如果制作方案在用户未要求时加入 BGM、旁白/配音或字幕，必须移除，并把 BGM、旁白/配音和字幕作为可选项询问。
