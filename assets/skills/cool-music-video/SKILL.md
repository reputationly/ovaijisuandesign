...
description: |
  根据歌曲、歌词、表演者参考或风格方向，制作一条十五秒以内的横屏音乐短片，完成人物与音乐来源确认、八至十个镜头、连续空间、动态字体、音乐结构、完整提示词和单条成片，适用于复古拼贴、说唱与时尚表演；不用于完整歌曲、多段长篇 MV、纯音乐分析或终剪合成。
trigger-words: [短MV, 音乐视频, 说唱MV, 时尚表演视频, 拼贴音乐视频, 十五秒MV]
---
...
- 用户只泛称 MV / 音乐视频 / music video，或只说单条但没有范围：只使用一次 确认卡，逐字使用：

```text
header: 成片范围
确认卡： 这条音乐视觉默认按完整 MV 规划，还是只做 15 秒以内单条测试体验？
options:
...
若只缺风格处理，确认卡 逐字使用；若还缺画面形态、人物来源或音乐内容，则把本子问题作为配置问询流程的第 1 题，一次性收集，不单独重复弹窗：

```text
header: 风格处理
确认卡： 当前 15 秒短 MV 模板主打“复古潮流拼贴”（我们觉得最稳定、效果最好）。你希望把你提到的风格融合进去，改做主打复古潮流拼贴效果，还是坚持纯外部风格？
options:
...
只在 `scope_confirmed=true` 后检查以下三项；同时把“风格提醒门”里未解决的风格处理一起纳入配置问询。多项缺失时只使用一次 确认卡 流程，不要连续弹多张卡片。这个流程应表现为一个弹窗 / 一个问询流程里的多个编号子问题，而不是把所有内容挤进一个长问题；按顺序展示：
...
只要 15 秒及以内短 MV 已确认，但用户没有明确画面形态，就在配置问询流程中加入本子问题。若只缺画面形态，确认卡 逐字使用：

```text
header: 画面形态
确认卡： 这条 15 秒短 MV 你想要真人实拍、真人结合动漫，还是纯动漫 / 二次元？
options:
...
派单前核对：两道前置门通过；真实 refs 与提示词槽位一致；歌词/声线与人物一致；至少 8 个 Shot、4 个连贯空间区段、5 个动作族、2 个设计主导镜头；多数文字镜头有逐镜 Layer A/B/C；音乐有变化和明确结尾；实际字符数合法；展示字符串与执行字符串字符级一致。未通过时只修当前字符串，不派摘要版。
