...
trigger-words: [教育视频, 教学视频, 动态讲题, 课程视频, 知识点视频, 教案, 课件, 练习题, 科普讲解, 课程逐字稿]
guide-prompt: |
...
description: |
  将主题、题目、课堂录音、逐字稿、教材或课程素材加工为可教学、可学习或可评量的内容，完成学习目标与路径设计、讲解和例题、课堂活动、练习与答案、课件讲义或教育视频制作，适用于课程、培训、科普和动态讲题；不用于普通修图、格式转换、无教育目的的转录或通用素材动效。
---
...
`Brief.md` 中先形成内容完整、语速自然的逐字旁白初稿。初稿完成后按受众对应的基准语速、有效字数和标点/呼吸停顿计算自然发声时长，并计入必要画面缓冲，得到当前脚本预计总时长。随后必须调用真实 确认卡：`按照当前脚本和自然语速，预计成片约为 X 秒。这个时长可以接受吗？`，固定提供三个方向：`接受 X 秒（推荐）`、`更多的秒数`、`更少的秒数`。`更多的秒数`注明会增加脚本字数和教学内容，`更少的秒数`注明会减少脚本字数并精简内容。用户选择更多或更少后，必须先修改 `Brief.md` 的逐字旁白，再重新计算并再次发起该时长确认；禁止只改总 duration、画面缓冲或语速。时长确认前不得进入正式逐片段脚本。
