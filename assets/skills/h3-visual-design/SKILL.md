...
description: |
  使用动态字体与空间包装、主体追踪框和拓扑图形，或真人实拍与二维手绘互动等明确技法，制作一条 H3 动态视觉短片，完成素材与文字确认、技法路线选择、提示词编译、生成和验收；适用于字体、Logo、口播包装、TD/CV 视觉和手绘融合，不用于静态设计、普通修图或只要求自然运动的通用视频。
trigger-words: [H3动态设计, 动态字体, 字体包装, 主体追踪图形, TD视觉, 手绘实拍融合, 视频视觉包装]
---
...
1. 由 execution layer 直接执行，不创建 Stage Execution Plan，不派 planner 或 executor 重新设计。
